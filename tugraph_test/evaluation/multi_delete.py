...
def parse_args():
    ...
    parser.add_argument('--create_num', help='create num')
...
def OptTest(folder_name):
    ...
    for j in range(0,cycle):
        for i in range(0,create_num):
            # 测试
            file_name="create_edge.cypher"
            create_file=os.path.join(cypher_folder,file_name)
            with open(create_file) as f:
                cypher_name=file_name.split(".")[0]
                input_cypher=f.read()
        ...
        for line in lines:
            print(line)
            single_original_time,single_optimized_time=test_cypher(line)
            original_time+=single_original_time
            optimized_time+=single_optimized_time

    f=open(output_path,"a")
    # for input_cypher, values in original_result_map.items():
    f.write(input_cypher+"\n")
    f.write("cycle: "+str(cycle)+"\n")
    f.write("create edge: "+str(create_num)+"\n")
    f.write("original_time: "+str(original_time/cycle)+"\n")
    f.write("optimized_time: "+str(optimized_time/cycle)+"\n")
    f.write("speed up: "+str(original_time/optimized_time)+"\n")
    f.write("\n")
...
if  __name__ == '__main__':
    ...
    if folder_name=='':
        folder_name=graph
    global output_path
    output_path=os.path.join(root_folder,folder_name,"multi_delete_log.txt")
    OptTest(folder_name)
