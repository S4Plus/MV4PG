...
def create_views(path,url,user,password):
    ...
    return createview_time
def delete_views(path,url,user,password):
    connector1=connector(url,user,password)
    with open(path+"/views/del.txt") as createview:
        lines=createview.readlines()
        for line in lines:
            print(line)
            connector1.run_cypher(line)
...
if __name__=="__main__":
    ...
    all_time={}
    for it in range(5):
        create_time=create_views(path,neo4j_url2,neo4j_user2,neo4j_password2)
        all_time[str(it)]=create_time
        delete_views(path,neo4j_url2,neo4j_user2,neo4j_password2)
    with open(path+"/result/views.json",'w') as inputfile:
        json.dump(all_time,inputfile,indent=4)