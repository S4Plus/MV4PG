...
//oC_MultiPartQuery : ( ( oC_ReadingClause SP? )* ( oC_UpdatingClause SP? )* oC_With SP? )+ oC_SinglePartQuery ;
    std::any visitOC_MultiPartQuery(LcypherParser::OC_MultiPartQueryContext *ctx) override {
        ...
        for(auto& mq:match_queries){
            if(_is_create){
            mq.append(" "+oc_with).append(",view_edge");
            mq.append(" "+update);
            }
            if(!_is_create){
            ...
            if(_is_vertex)
            {
                mq.append(" "+oc_with).append(",collect(view_vetex) as view_vetexs");
                mq.append(" "+oc_with).append(",size(view_vetexs) as num,view_vetexs");
                mq.append(" "+update);
                mq.append(" with collect(").append(view_variable_).append(")").append(" as views");
                std::string new_with(oc_with.begin()+4,oc_with.end());
                mq.append(","+new_with).append(",num,view_vetexs");
                mq.append(" UNWIND range(0,num-1) as i delete views[i]");
            }
            else{
                mq.append(" "+oc_with).append(",collect(view_edge) as view_edges");
                mq.append(" "+oc_with).append(",size(view_edges) as num,view_edges");
                mq.append(" "+update);
                mq.append(" with collect(").append(view_variable_).append(")").append(" as views");
                std::string new_with(oc_with.begin()+4,oc_with.end());
                mq.append(","+new_with).append(",num,view_edges");
                mq.append(" UNWIND range(0,num-1) as i delete views[i]");
            }
            }//mp.append("collect(r) as view delete view[0]")
...
// oC_Create : CREATE SP? oC_Pattern ;
    std::any visitOC_Create(LcypherParser::OC_CreateContext *ctx) override {
        ...
        if(_is_create) return visitChildrenToString(ctx);
            ...
            std::string result="optional match "+pattern;
            return result;
        }
        _LeaveClauseCREATE();
    }
                                ...
                                for(int i=0;i<result.size();i++){
                                    if(result[i]=='['){
                                        for(int j=i+1;j<result.size();j++){
                                            if(result[j]==']'){
                                                std::string var(result.begin()+i+1,result.begin()+j);
                                                if(!var.empty()){
                                                if(isfirst) result.append(" WHERE ");
                                                else result.append(" AND ");
                                                isfirst=false;
                                                result.append(var).append(".is_view is null ");
                                            }
                                            break;
                                            }
                                            if(result[j]==':')break;
                                            if(result[j]=='*'){
                                                ...
                                                isfirst=false;
                                                result.append("ALL(rel IN ").append(var);
                                                result.append(" WHERE rel.is_view is null) ");
...
//oC_NodePattern : '(' SP? ( oC_Variable SP? )? ( oC_NodeLabels SP? )? ( oC_Properties SP? )? ')' ;
  std::any visitOC_NodePattern(LcypherParser::OC_NodePatternContext *ctx) override {
        ...
        result.append(")");
         if(_change&&_is_vertex){
            result.append("-[*0]-").append("(view_vetex)");
            }
        return result;
    }
std::string getvetex() {
    std::string result{ "(" };
    result.append("view_vetex");
    result.append(")");
    return result;
}
            ...
            return unfold_relps;
               ...
               if(max_hop==VAR_LEN_EXPAND_MAX_HOP)
               {
                 ...
                 for(int i=1;i<min_hop;i++)
                 {
                   ...
                   path.append("[");
                   path.append(GenAnonymousAlias(false));
                   ...
                   path1.append("[");
                   path1.append(GenAnonymousAlias(false));
            ...
            if(relp_pattern->oC_RelationshipDetail()->oC_RangeLiteral()==nullptr)return unfold_relps;
                ...
                if(max_hop==VAR_LEN_EXPAND_MAX_HOP)
                {
                    for(int i=0;i<min_hop-1;i++)
                    {
                       ...
                       path.append(getedge(egdetypes,left_arrow,right_arrow));//获得边(:person{id:1})-[r:write]->(:movie{id:2})
                       path.append(left_arrow).append("[");
                       path.append(GenAnonymousAlias(false));
                    ...
                    path1.append(getedge(egdetypes,left_arrow,right_arrow));//获得边(:person{id:1})-[r:write]->(:movie{id:2})
                    path1.append(left_arrow).append("[");
                       path1.append(GenAnonymousAlias(false));
                    ...
                    for(int i=0;i<min_hop;i++)
                    {
                       ...
                       path.append(GenAnonymousAlias(false));
                        ...
                        path.append("*");
                        path.append(index1);
                        path.append("]").append(right_arrow).append(getedge(egdetypes,left_arrow,right_arrow));
                        path.append(left_arrow).append("[");
                       path.append(GenAnonymousAlias(false));
                    ...
                    path1.append("]").append(right_arrow).append(getedge(egdetypes,left_arrow,right_arrow));
                    path1.append(left_arrow).append("[");
                       path1.append(GenAnonymousAlias(false));
        ...
        //////////////////
            ...
            if(_change){
                ...
                // result.append(":"+_edge_label);
                   result.append(std::any_cast<std::string>(visit(ctx->oC_RelationshipTypes())));
                }
                if(ctx->oC_RangeLiteral()!=nullptr)
...
std::string getedge(std::string edgetypes,std::string left_rrow,std::string right_rrow) {
    std::string result;
    std::string node1{ "()" };
    std::string edge("[");
    std::string node2{ "()" };
    ...
    // edge.append("viewedge").append(":").append(_edge_label).append("]");
    if(edgetypes.empty())edge.append("view_edge").append("]");
    else 
        edge.append("view_edge").append(":").append(edgetypes).append("]");
    result.append(node1).append(left_rrow).append(edge).append(right_rrow).append(node2);
    return result;
}
